...
class BertForEventExtract(BaseModel):
    def __init__(self, hparams: Hparams, **kwargs):
        ...
        self.trigger_cnn_encoder = DgcnnBlock(self.hidden_size, [3, 3, 3], [1, 2, 4], name="trigger_dgcnn_encoder")
        self.role_cnn_encoder = DgcnnBlock(self.hidden_size, [3, 3, 3], [1, 2, 4], name="role_dgcnn_encoder")
        ...
        self.trigger_crf = CRFLayer(self.trigger_num_labels, self.initializer_range, name="trigger_crf_output")
        self.role_crf = CRFLayer(self.role_num_labels, self.initializer_range, name="role_crf_output")
    ...
    def call(self, inputs, **kwargs):
        ...
        # dgcnn encoding
        trigger_fusion = self.trigger_cnn_encoder(trigger_project, training=training)
        role_fusion = self.role_cnn_encoder(role_project, training=training)
        ...
        trigger_viterbi, _ = self.trigger_crf([trigger_logits, input_lengths])
        role_viterbi, _ = self.role_crf([role_logits, input_lengths])

        trigger_logits = tf.keras.backend.in_train_phase(trigger_logits, tf.one_hot(trigger_viterbi, self.trigger_num_labels), training=training)
        role_logits = tf.keras.backend.in_train_phase(role_logits, tf.one_hot(role_viterbi, self.role_num_labels), training=training)
