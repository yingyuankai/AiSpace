...
def create_awdwwu_optimizer(hparams):
    """Creates an optimizer with learning rate schedule."""
    init_lr, num_train_steps, num_warmup_steps = \
        hparams.learning_rate, hparams.steps_per_epoch * hparams.max_epochs, hparams.num_warmup_steps
...
class AdamWeightDecay(tf.keras.optimizers.Adam):
  ...
  """
    ...
    def _decay_weights_op(self, var, learning_rate, apply_state):
        ...
        if do_decay:
            if apply_state is None:
                cur_wdr = tf.constant(self.weight_decay_rate, name="adam_weight_decay_rate")
            else:
                cur_wdr = apply_state[(var.device, var.dtype.base_dtype)]["weight_decay_rate"]
            return var.assign_sub(
                learning_rate * var * cur_wdr,
                use_locking=self._use_locking,
            )
        return tf.no_op()
