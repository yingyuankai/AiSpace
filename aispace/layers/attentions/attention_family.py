...
Ref: https://github.com/uzaymacar/attention-mechanisms
"""
...
class Attention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if 'local-p' in self.alignment_type:
            ...
            self.v_p = tf.keras.layers.Dense(units=1, use_bias=False)
    ...
    def call(self, inputs):
        ...
        if self.alignment_type == 'global':                                                         # Global Approach
            ...
            if self.alignment_type == 'local-m':                                                    # Monotonic Alignment
                ...
                source_hidden_states = tf.keras.layers.Lambda(lambda x: x[:, left:right, :])(source_hidden_states)  # (B, S*=(D, 2xD), H)

            elif self.alignment_type == 'local-p':                                                  # Predictive Alignment
                aligned_position = self.W_p(target_hidden_state)                                    # (B, 1, H)
                aligned_position = tf.keras.layers.Activation('tanh')(aligned_position)                             # (B, 1, H)
                aligned_position = self.v_p(aligned_position)                                       # (B, 1, 1)
                aligned_position = tf.keras.layers.Activation('sigmoid')(aligned_position)                          # (B, 1, 1)
                ...
                aligned_position = tf.keras.layers.Activation('tanh')(aligned_position)                             # (B, S, H)
                aligned_position = self.v_p(aligned_position)                                       # (B, S, 1)
                aligned_position = tf.keras.layers.Activation('sigmoid')(aligned_position)                          # (B, S, 1)
                ...
                aligned_position = tf.keras.layers.Multiply()([aligned_position, onehot_vector])                    # (B, S)
                aligned_position = tf.expand_dims(aligned_position, axis=-1)                        # (B, S, 1)
                initial_source_hidden_states = source_hidden_states                                 # (B, S, 1)
                source_hidden_states = tf.keras.layers.Multiply()([source_hidden_states, aligned_position])         # (B, S*=S(D), H)
        ...
        if 'dot' in self.score_function:                                                            # Dot Score Function
            attention_score = tf.keras.layers.Dot(axes=[2, 2])([source_hidden_states, target_hidden_state])         # (B, S*, 1)
            ...
            attention_score = tf.keras.layers.Dot(axes=[2, 2])([weighted_hidden_states, target_hidden_state])       # (B, S*, 1)

        elif self.score_function == 'location':                                                     # Location-based Score Function
            weighted_target_state = self.W_a(target_hidden_state)                                   # (B, 1, H)
            attention_score = tf.keras.layers.Activation('softmax')(weighted_target_state)                          # (B, 1, H)
            attention_score = tf.keras.layers.RepeatVector(source_hidden_states.shape[1])(attention_score)          # (B, S*, H)
            ...
            weighted_sum = tf.keras.layers.Activation('tanh')(weighted_sum)                                         # (B, S*, H)
            attention_score = self.v_a(weighted_sum)                                                # (B, S*, 1)

        # Compute attention weights
        attention_weights = tf.keras.layers.Activation('softmax')(attention_score)                                  # (B, S*, 1)
        ...
        if self.alignment_type == 'local-p':                                                        # Gaussian Distribution
            ...
            for i in range(1, self.input_sequence_length):
                gaussian_factor = tf.keras.layers.Concatenate(axis=1)([gaussian_factor, gaussian_estimation(i)])    # (B, S*, 1)
...
class SelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):  # (B, S, H)
        ...
        hidden_states_transposed = tf.keras.layers.Permute(dims=(2, 1))(inputs)                                     # (B, H, S)
        attention_score = tf.matmul(W1, hidden_states_transposed)                                   # (B, size, S)
        attention_score = tf.keras.layers.Activation('tanh')(attention_score)                                       # (B, size, S)
        attention_weights = tf.matmul(W2, attention_score)                                          # (B, num_hops, S)
        attention_weights = tf.keras.layers.Activation('softmax')(attention_weights)                                # (B, num_hops, S)
        embedding_matrix = tf.matmul(attention_weights, inputs)                                     # (B, num_hops, H)
        embedding_matrix_flattened = tf.keras.layers.Flatten()(embedding_matrix)                                    # (B, num_hops*H)

        if self.use_penalization:
            attention_weights_transposed = tf.keras.layers.Permute(dims=(2, 1))(attention_weights)                  # (B, S, num_hops)
