...
class DuEERoleTransformer(BaseTransformer):
        ...
        with open(data_path, "r", encoding="utf8") as inf:
            with open(output_path, "w", encoding="utf8") as ouf:
                for line in tqdm(inf):
                    ...
                    line_json = json.loads(line)
                    if len(line_json) == 0: continue
                    features = self._build_featureV2(line_json, schema, label2ids)
    ...
    def _build_featureV2(self, one_json, schema, label2ids):
        ...
        for event in one_json.get("event_list"):
            ...
            # trigger_labels = ["O"] * len(pre_trigger_tokens) + ["B-mention"] + ["I-mention"] * (len(trigger_tokens) - 1)
            # post_trigger_tokens = self.tokenizer.tokenize(text[trigger_start_index + len(trigger): ])
            # trigger_labels += ["O"] * (len(post_trigger_tokens))
