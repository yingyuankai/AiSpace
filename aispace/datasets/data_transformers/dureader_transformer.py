...
@BaseTransformer.register("dureader/robust")
class DuReaderRobustTransformer(BaseTransformer):
    def __init__(self, hparams, **kwargs):
        super(DuReaderRobustTransformer, self).__init__(hparams, **kwargs)
            ...
            for entry in data_list:
                for paragraph in entry['paragraphs']:
                    ...
                    for qa in paragraph['qas']:
                        ...
                        is_impossible = int(qa.get("is_impossible", len(qa.get('answers', [])) == 0))

                        if (len(qa.get('answers', [])) == 0) and (not is_impossible):
