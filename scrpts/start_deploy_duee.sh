...
TF_FORCE_GPU_ALLOW_GROWTH=true CUDA_VISIBLE_DEVICES=5 bentoml serve $TRIGGER_DEPLOY_PATH > trigger_extract_deploy.log 2>&1 &
echo "Start trigger extract service."

export BENTOML__APISERVER__DEFAULT_PORT=5001
TF_FORCE_GPU_ALLOW_GROWTH=true CUDA_VISIBLE_DEVICES=5 bentoml serve $ROLE_DEPLOY_PATH > role_extract_deploy.log 2>&1 &
echo "Start role extract service."

export BENTOML__APISERVER__DEFAULT_PORT=5002
TF_FORCE_GPU_ALLOW_GROWTH=true CUDA_VISIBLE_DEVICES=6 bentoml serve $EVENT_TYPE_DEPLOY_PATH > event_type_extract_deploy.log 2>&1 &
